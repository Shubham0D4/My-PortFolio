(function (factory) {
	    ...
	    if (!isNaN(val.min) && !isNaN(val.max)) {
	      ...
	      if (drawText) {
	        ...
	        if (isCue) {
	          ...
	          var float = start !== (start | 0);
	          var fixedDigits = float ? Math.min(5, Math.max(1, Math.floor(bounds.w / 10 - 1))) : 0;
	...
	var TimelineChart = /*#__PURE__*/function () {
	  ...
	  _proto.updateLevelOrTrack = function updateLevelOrTrack(details) {
	    ...
	    var fragments = (details.fragments || []).map(function (fragment) {
	    ...
	    var parts = details.partList ? [].concat(details.partList.map(function (part) {
	      return _objectSpread2(_objectSpread2({
	        dataType: 'part'
	      }, part), {}, {
	        start: part.fragment.start + part.fragOffset,
	        fragment: _objectSpread2(_objectSpread2({}, part.fragment), {}, {
	          loader: null
	        })
	      });
	    }), details.fragmentHint ? [_objectSpread2(_objectSpread2({
	      dataType: 'fragmentHint'
	    }, details.fragmentHint), {}, {
	      loader: null
	    })] : []) : [];
	    levelDataSet.data = [].concat(fragments, parts);
	...
	;
	  ...
	  _proto.drawCurrentTime = function drawCurrentTime() {
	    ...
	    if ((_self$hls3 = self.hls) != null && _self$hls3.media && chart.data.datasets.length) {
	      ...
	      var scale = this.chartScales[X_AXIS_SECONDS];
	      var ctx = chart.ctx;
	      if (this.hidden || !(ctx != null && ctx.canvas.width)) {
	        return;
	      }
	      var chartArea = chart.chartArea;
	...
	var testStreams = requireTestStreams();
	...
	var demoConfig = {};
	var demoConfigParam = getURLParam('demoConfig', null);
	if (demoConfigParam) {
	  try {
	...
	var hlsjsConfigParam = getURLParam('hlsjsConfig', null);
	var hlsjsConfig = hlsjsDefaults;
	var hlsjsConfigLoadedFromUrl = false;
	if (hlsjsConfigParam) {
	  try {
	...
	function getMetrics() {
	  ...
	  var jsonpacked = mainExports.pack(json);
	...
	function updateAudioTrackInfo() {
	  ...
	  var nextAudioTrackId = hls.nextAudioTrack;
	  var len = hls.audioTracks.length;
	  var track = hls.audioTracks[audioTrackId];
	  var nextTrack = hls.audioTracks[nextAudioTrackId];
	  for (var i = 0; i < len; i++) {
	  ...
	  for (var _i = 0; _i < len; _i++) {
	    ...
	    if (nextAudioTrackId === _i) {
	      html2 += buttonEnabled;
	    } else {
	      html2 += buttonDisabled;
	    }
	    html2 += 'onclick="hls.nextAudioTrack=' + _i + '">' + hls.audioTracks[_i].name + '</button>';
	  }
	  $('#audioTrackLabel').text(track ? track.lang || track.name : 'None selected');
	  $('#audioTrackControl').html(html1);
	  $('#audioTrackLabel').text(nextTrack ? nextTrack.lang || nextTrack.name : 'None selected');
	...
	function updatePermalink(firstLoad) {
	  ...
	  var serializedHlsjsConfig = btoa(JSON.stringify(hlsjsConfig));
	  var baseURL = document.URL.split('?')[0];
	  var streamURL = $('#streamURL').val();
	  var permalinkURL = baseURL + "?src=" + encodeURIComponent(streamURL) + "&demoConfig=" + serializedDemoConfig + "&hlsjsConfig=" + serializedHlsjsConfig;
	...
	function setupConfigEditor() {
	  ...
	  var contents = hlsjsConfig;
	  var shouldRestorePersisted = !hlsjsConfigLoadedFromUrl && JSON.parse(localStorage.getItem(STORAGE_KEYS.Editor_Persistence)) === true;
